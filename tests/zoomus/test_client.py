...
class ZoomClientTestCase(unittest.TestCase):
    ...
    def test_invalid_api_version_raises_error(self):
        with self.assertRaisesRegexp(RuntimeError, "API version not supported: 42"):
            ZoomClient("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET", version=42)

    def test_init_sets_config_with_timeout(self):
        client = ZoomClient("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET", timeout=500)
        self.assertEqual(client.timeout, 500)

    def test_cannot_init_with_non_int_timeout(self):
        with self.assertRaises(ValueError) as context:
            ZoomClient("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET", timeout="bad")
            self.assertEqual(
    ...
    def test_can_set_api_version_to_1(self):
        client = ZoomClient("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET", version=util.API_VERSION_1)
    ...
    def test_can_set_base_uri_to_gdpr(self):
        client = ZoomClient("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET", base_uri=API_BASE_URIS[util.API_GDPR])
        self.assertEqual(client.config["version"], util.API_VERSION_2)
        for key in client.components.keys():
            self.assertEqual(
    ...
    def test_can_set_custom_base_uri(self):
        client = ZoomClient(
            "ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET", version=util.API_VERSION_1, base_uri="https://www.test.com"
        )
        self.assertEqual(client.config["version"], util.API_VERSION_1)
        for key in client.components.keys():
    ...
    def test_can_get_account_id(self):
        client = ZoomClient("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET")
        self.assertEqual(client.account_id, "ACCOUNT_ID")

    def test_can_set_account_id(self):
        client = ZoomClient("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET")
        client.account_id = "NEW-ACCOUNT-ID"
        self.assertEqual(client.account_id, "NEW-ACCOUNT-ID")
    ...
    def test_can_use_client_with_context(self):
        with ZoomClient("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET") as client:
    ...
    def test_refresh_token_replaces_config_token_with_new_access_token(
        ...
        mock_access_token.assert_called_with("ACCOUNT_ID", "CLIENT_ID", "CLIENT_SECRET")
