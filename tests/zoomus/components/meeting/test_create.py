...
class CreateV1TestCase(unittest.TestCase):
    ...
    def test_can_create(self):
        responses.add(
            responses.POST,
            "http://foo.com/meeting/create?host_id=ID&topic=TOPIC&type=TYPE&account_id=KEY&client_id=SECRET",
        )
    ...
    def test_does_convert_startime_to_str_if_datetime(self):
        responses.add(
            responses.POST,
            "http://foo.com/meeting/create?host_id=ID&topic=TOPIC&type=TYPE&account_id=KEY&client_id=SECRET&start_time=2020-01-01T01%3A01%3A00Z",
        )
